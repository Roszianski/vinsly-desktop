...
const ActionMenu: React.FC<ActionMenuProps> = ({ command, onEdit, onDuplicate, onDelete }) => {
  ...
  const handleReveal = async () => {
    if (!command.path || isRevealing) return;
    setIsRevealing(true);
    try {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && menuCoords && createPortal(
        ...
        >
          <button
            onClick={() => handleAction(() => onEdit(command))}
          ...
          {onDuplicate && (
            <button
              onClick={() => handleAction(() => onDuplicate(command))}
              className="w-full px-4 py-2.5 text-left text-sm text-v-light-text-primary dark:text-v-text-primary hover:bg-v-light-hover dark:hover:bg-v-light-dark transition-colors flex items-center gap-3"
            >
          ...
          <button
            onClick={() => handleAction(() => onDelete(command))}
...
interface SlashCommandListScreenProps {
  commands: SlashCommand[];
  onCreate: () => void;
  onEdit: (command: SlashCommand) => void;
  onDuplicate?: (command: SlashCommand) => void;
  onDelete: (commandId: string) => void;
  onBulkDelete?: (commandIds: string[]) => void;
  onToggleFavorite: (command: SlashCommand) => void;
  onImport?: () => void;
  onExport?: (commands: SlashCommand[]) => void;
...
export const SlashCommandListScreen: React.FC<SlashCommandListScreenProps> = ({
  ...
  const filteredCommands = useMemo(() => {
    ...
    // Filter by scope
    if (filter !== 'All') {
      result = result.filter(cmd => cmd.scope === filter);
    }
    ...
    if (searchQuery.trim()) {
      result = result.filter(cmd => {
        ...
        return fuzzyMatch(searchText, searchQuery.toLowerCase());
      });
    }
    ...
    return result;
  }, [commands, filter, searchQuery, sortCriteria]);

  // Selection state
  const allFilteredSelected = filteredCommands.length > 0 && filteredCommands.every(c => selectedCommandIds.has(c.id));
  const someFilteredSelected = filteredCommands.some(c => selectedCommandIds.has(c.id));
  ...
  }, [filter, searchQuery, layoutMode]);

  const handleSelectAll = () => {
    if (allFilteredSelected) {
      const newSelection = new Set(selectedCommandIds);
      filteredCommands.forEach(c => newSelection.delete(c.id));
      setSelectedCommandIds(newSelection);
    } else {
      const newSelection = new Set(selectedCommandIds);
      filteredCommands.forEach(c => newSelection.add(c.id));
      setSelectedCommandIds(newSelection);
    }
  };

  const handleSelectOne = (commandId: string) => {
    const newSelection = new Set(selectedCommandIds);
  ...
  const handleConfirmDelete = () => {
    if (deleteTarget === 'single' && commandToDelete) {
      onDelete(commandToDelete.id);
    } else if (deleteTarget === 'bulk' && onBulkDelete) {
      onBulkDelete(Array.from(selectedCommandIds));
      setSelectedCommandIds(new Set());
    }
  ...
  const handleExportSelected = () => {
    if (onExport) {
      const selectedCommands = commands.filter(c => selectedCommandIds.has(c.id));
      onExport(selectedCommands);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: 'Total commands', value: commands.length, icon: TerminalIcon },
          { label: 'Global commands', value: commands.filter(c => c.scope === AgentScope.Global).length, icon: GlobeIcon },
          { label: 'Project commands', value: commands.filter(c => c.scope === AgentScope.Project).length, icon: FolderIcon },
        ].map(stat => {
        ...
        <div className="flex items-center gap-2">
          {selectedCommandIds.size > 0 ? (
            ...
            <>
              ...
              {onExport && (
                <button
                  onClick={() => onExport(filteredCommands)}
          ...
          <div className="divide-y divide-v-light-border dark:divide-v-border">
            ...
            >
              ...
              <div className="text-right">
                <label htmlFor="command-sort" className="sr-only">Sort commands by</label>
      ...
      <ConfirmDialog
        ...
        title={deleteTarget === 'bulk' ? 'Delete Commands' : 'Delete Command'}
        message={
          deleteTarget === 'bulk'
            ? `Are you sure you want to delete ${selectedCommandIds.size} command(s)? This action cannot be undone.`
