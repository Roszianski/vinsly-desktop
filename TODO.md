...
## Phase 3 – Security & performance hardening (tomorrow)
- Tighten Tauri capabilities to only the FS/dialog scopes the app actually uses; drop shell access in production builds.
- Move home-directory discovery onto a blocking worker/cached background task and add simple cancellation/debounce so large disks don’t freeze the UI when rescanning.
